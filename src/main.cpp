...
// Implemented https://github.com/phoreproject/Phore/pull/133 (Fix block spam) updated to #136
...
#if defined(NDEBUG)
#error "mqx cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zmqx spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zmqx spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zmqx spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 0 * COIN;
    }	else if (nHeight <= 250000 && nHeight > 50) {
        nSubsidy = 4 * COIN;
        ...
        nSubsidy = 2.6 * COIN;
    }	else if (nHeight <= 1500000 && nHeight > 1250000) {
        nSubsidy = 2.4 * COIN;
    }	else if (nHeight <= 1750000 && nHeight > 1500000) {
        nSubsidy = 2.2 * COIN;
    }	else if (nHeight <= 2000000 && nHeight > 1750000) {
        nSubsidy = 2 * COIN;
    }	else if (nHeight <= 2250000 && nHeight > 2000000) {
        nSubsidy = 1.8 * COIN;
    }	else if (nHeight <= 2500000 && nHeight > 2250000) {
        nSubsidy = 1.6 * COIN;
    }	else if (nHeight <= 2750000 && nHeight > 2500000) {
        nSubsidy = 1.4 * COIN;
    }	else if (nHeight <= 3050000 && nHeight > 2750000) {
        nSubsidy = 1.2 * COIN;
    }	else if (nHeight <= 3375000 && nHeight > 3050000) {
        nSubsidy = 1 * COIN;
    }	else if (nHeight <= 3750000 && nHeight > 3375000) {
        nSubsidy = 0.8 * COIN;
    }	else if (nHeight <= 4200000 && nHeight > 3750000) {
        nSubsidy = 0.6 * COIN;
    }	else if (nHeight <= 5063000 && nHeight > 4200000) {
        nSubsidy = 0.4 * COIN;
    }	else if (nHeight > 5063000) {
        nSubsidy = 0 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from mqx
...
{
    RenameThread("mqx-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZMQXMinted()
{
...
void RecalculateZMQXSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zmqx supply
...
bool RecalculateMQXSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zmqx with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zmqxSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert MQX to zmqx
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zmqx spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zmqx serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zmqx mints and %d zmqx spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // mqx: We use certain sporks during IBD, so check to see if they are
