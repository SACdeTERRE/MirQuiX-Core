...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total mqx balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"zmqxsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zmqx denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zmqx denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zmqx denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zmqx denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zmqx denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zmqx denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zmqx denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zmqx denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zmqx denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,    (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,        (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,      (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in mqx/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in mqx/kb\n"
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zmqxObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zmqxObj.emplace_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.emplace_back(Pair("zmqxsupply", zmqxObj));
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"mqxaddress\"\n"
            "\nReturn information about the given mqx address.\n"
            "\nArguments:\n"
            "1. \"mqxaddress\"     (string, required) The mqx address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"mqxaddress\", (string) The mqx address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: mqx address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are mqx addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) mqx address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"mqxaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"mqxaddress\"  (string, required) The mqx address to use for the signature.\n"
