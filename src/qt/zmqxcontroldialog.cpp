...
#include "ui_zmqxcontroldialog.h"
...
std::list<std::string> ZmqxControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZmqxControlDialog::listMints;

ZmqxControlDialog::ZmqxControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZmqxControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZmqxControlDialog::~ZmqxControlDialog()
{
    delete ui;
}

void ZmqxControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZmqxControlDialog::updateList()
{
...
void ZmqxControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZmqxControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZmqx_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZmqxControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZmqxControlDialog::GetSelectedMints()
{
...
void ZmqxControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
