                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zmqx ought to be enough for anybody." - Bill Gates, 2017
    ui->zmqxpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzmqxSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzmqx->setEnabled(false);
        ui->pushButtonMintzmqx->setToolTip(tr("zmqx is currently disabled due to maintenance."));

        ui->pushButtonSpendzmqx->setEnabled(false);
        ui->pushButtonSpendzmqx->setToolTip(tr("zmqx is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zmqxpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzmqx_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zmqx is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zmqx...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zmqx in ") +
...
void PrivacyDialog::on_pushButtonSpendzmqx_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zmqx is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zmqx
        sendzmqx();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zmqx
    sendzmqx();
}

void PrivacyDialog::on_pushButtonZmqxControl_clicked()
{
    ZmqxControlDialog* zmqxControl = new ZmqxControlDialog(this);
    zmqxControl->setModel(walletModel);
    zmqxControl->exec();
}

void PrivacyDialog::setZmqxControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzmqxSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzmqx()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid mqx Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zmqxpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zmqx is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " MQX </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zmqx</b>";
    ...
    // use mints from zmqx selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZmqxControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZmqxControlDialog::GetSelectedMints();
    }
    ...
    // Clear zmqx selector in case it was used
    ZmqxControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zmqx Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " mqx, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zmqx Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zmqx </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zmqx "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zmqx "));
    ui->labelzmqxAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzmqxSyncStatus->setVisible(fShow);
}
