...
bool ComputeNextStakeModifier(const CBlockIndex* pindexPrev, uint64_t& nStakeModifier, bool& fGeneratedStakeModifier)
{
    ...
    if (pindexPrev->nHeight <= 51) {
        //Give stakemodifiers to the all POW blocks +1
        fGeneratedStakeModifier = true;
...
{
    //mqx will hash in the transaction hash and the index number in order to make sure each hash is unique
